...
2. 我们只能通过观测函数 $h(\cdot)$ 看到状态的一部分（返回值/ack/错误等），而不是状态本身。
...
2. **基于探测结果做条件化查询/过滤**（只对可用子集做筛选）
3. **评估返回结果是否可接受**（partial 是否要继续补全）
4. **前端高亮并读取 ack**（可能部分失败）
5. 如有必要继续补救（补全/重试/降级）
...
2. 两种范式的核心差异在于“动作表示”与“验证时机”
3. 成本函数负责累加代价，结构参数（如交互强度、约束强度）决定代价如何增长
4. 在 digital twin 的典型交互任务中，应当将两者视为互补组件，而非单选题

在[下篇](https://yosgi.github.io/zh/post/%E7%BB%93%E6%9E%84%E6%80%A7%E5%BF%85%E9%A1%BB-%E6%95%B0%E5%AD%97%E5%AD%AA%E7%94%9F%E7%9A%84ai-%E6%B7%B7%E5%90%88%E6%9E%B6%E6%9E%84/)中，我会继续记录我如何尝试一种折中架构，在不放弃 MCP-as-Code 优势的前提下，把延迟和失败成本重新拉回可接受区间。
